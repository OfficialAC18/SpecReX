...
from ReX.config import CausalArgs
...
class Explanation(Base):
    ...
    time = Column(Float)
    ranking_type = Column(RankingType)
...
def initialise_rex_db(name, echo=False):
    ...
    Base.metadata.create_all(engine, tables=[Explanation.__table__], checkfirst=True)  # type: ignore
    Session = sessionmaker(bind=engine)
    s = Session()
    return s


def dataframe(db, table="rex"):
    df = pd.read_sql_table(table, f"sqlite:///{db}")

    # df["pixel_ranking"] = np.frombuffer(zlib.decompress(df["pixel_ranking"]), dtype=np.float32).reshape(
    #     (df["px_shape_x"], df["px_shape_y"])
    # )
    # df["pixel_ranking"] = df["pixel_ranking"].apply(_to_numpy)
