...
class BoxInternal:
    ...
    def spawn_children(self, min_size, invert, pixel_ranking=None) -> List[Box]:
        """split a box into 4 contiguous sections"""
        if self.length() < min_size:
            return []
        
        if self.distribution == Distribution.Adaptive and pixel_ranking is not None:
            l = pixel_ranking.shape
            mask = np.zeros(l)
            mask[self.row_start : self.row_stop] = pixel_ranking[self.row_start : self.row_stop]
...
class Box(BoxInternal, NodeMixin):
    ...
    def add_children_to_tree(self, min_size, invert, pixel_ranking=None):
        if not self.children:
            self.children = self.spawn_children(min_size, pixel_ranking=pixel_ranking, invert=invert)
...
def build_tree(root, depth, min_size, pixel_ranking=None, invert=True) -> None:
    for n in PreOrderIter(root):
        if n.depth <= depth and len(n.children) == 0:
            n.add_children_to_tree(min_size, invert, pixel_ranking=pixel_ranking)
