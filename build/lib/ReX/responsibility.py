...
def set_held(tree, explanation, held) -> None:
    ...
    for b_name in held:
        ...
        if box is not None:
            box.interpolate_mask(explanation)
...
def causal_explanation(
    ...
    while flag:
        ...
        for job in queue:
            mask = spectra_mask_multi(spec_array) #Creates a mask which is a replica of the original spectra
            for processing in job:
                held = [p for p in job if p not in [processing]]
                children = subbox(tree, processing)
                children = list(filter(lambda child: child.length() >= args.min_box_length, children)) #Can we avoid creating them in the first place?
                ...
                for i in range(14):
                    ...
                    if spec_shape.order == "first":
                        ...
                        mutants.append(mask)
                    partitions.append(partition)
                    mask = spectra_mask_multi(spec_array)
        ...
        for idx,mutant in enumerate(mutants):
            ...
            im = cv2.imwrite(os.path.join(MUTANT_PATH,f"mutant{idx}.jpeg"),
                            np.squeeze(mutant,0) * 255)
